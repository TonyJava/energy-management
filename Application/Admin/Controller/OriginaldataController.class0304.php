  ...
  */
        ...
        if($regionid){
            ...
            if($endtime && $starttime){
               ...
               $olddata=date("Y-m-d H:i:s",strtotime("-6 day"));
               $sql_select = $this->buildSql($sql_select, "t.data_dt>='".$olddata."'");
            }
	...
	/*	$arr=array(
		...
		*/
		       $rgn_name["name"]="设备名";
			   $rgn_name["value"]="rgn_name";
			   array_push($arr, $rgn_name);
		       $dev_code["name"]="设备编号"; 
			   $dev_code["value"]="dev_code";
			   array_push($arr, $dev_code);
		       $d2d_dt["name"]="时间";
			   $d2d_dt["value"]="dt";
			   array_push($arr, $d2d_dt);
		...
		foreach($arr as $ke=>$va){
			if(!in_array($ke['value'],$otherFields)) {
					$arrvalue['data_'.$va['value']] = $va['name'];
      ...
      foreach($Result as $orderkey=>$orderval){
			...
			foreach($arr  as $key=>$val){
		      if($val['value']=='rgn_name'){
				  $data[$orderkey]['rgn_name']=$orderval['rgn_name'];
			  }
			  if($val['value']=='dev_code'){
				  $data[$orderkey]['dev_code']=$orderval['dev_code'];
			  }
			  if($val['value']!='dev_code' && $val['value']!='rgn_name'){
				$data[$orderkey]['data_'.$val['value']]=$orderval['data_'.$val['value']];
			  }
			}
        }   
		
       array_unshift($data,$arrvalue);
