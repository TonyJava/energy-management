...
//         dump($monthaccu);
	...
	public function exp(){
		...
		$expdata=array();
	    foreach($Result as $key=>$val){
		    $expdata[$key]['time'] = $val['time'];
			$expdata[$key]['rgn_name'] = $val['rgn_name'];
			$expdata[$key]['d2de_dt'] = $val['d2de_dt'];
			$expdata[$key]['value_sysl'] = $val['value_sysl'];
			$expdata[$key]['sumsysl'] =	$val['sumsysl'];
			$expdata[$key]['value_ynl'] = $val['value_ynl'];
		}
		...
		array_unshift($expdata,$exparray);
		
        foreach ($expdata as $i => $row) {
            foreach ($row as $j => $v) {
