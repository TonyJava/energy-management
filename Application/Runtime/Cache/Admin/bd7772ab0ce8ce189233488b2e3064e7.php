			...
			<div class="row row-lg">
				<div class="col-sm-12">
					<div id="atpbiztoolbar">
                        <label class="control-label">【计划完成时间】小于【当前时间】且【处理状态】处于【进行中】的维修单</label>
	                </div>
	                <table id="atpbiztable" style="width:1300px;"></table>
				</div>
			</div>
		</div>
	</div>
...
<script>
	...
	$(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/RgRepairDelay/getData?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=<?php echo ($_GET['regionlevel']); ?>&tabindex=<?php echo ($_GET['tabindex']); ?>&rgn_atpid=<?php echo ($_GET['rgn_atpid']); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            cardView: true,                    //是否显示详细视图
            ...
            columns: [
                [
                    ...
                    {field: 'emp_name', title: '指派维修人', align:'center', valign:'middle',sortable: false},
                    {field: 'empuser', title: '配合人', align:'center', valign:'middle',sortable: false},

//                    {field: 'status', title: '状态', align:'center', valign:'middle',sortable: false},
//                    {field: 'revempid', title: '接单人', align:'center', valign:'middle',sortable: false},
//                    {field: 'revdate', title: '接单时间', align:'center', valign:'middle',sortable: false},
//                    {field: 'revplandate', title: '接单人计划完成时间', align:'center', valign:'middle',sortable: false},
//                    {field: 'revremark', title: '接单备注', align:'center', valign:'middle',sortable: false},
//                    {field: 'disposeempid', title: '处理人', align:'center', valign:'middle',sortable: false},
//                    {field: 'disposedate', title: '处理时间', align:'center', valign:'middle',sortable: false},
//                    {field: 'disposeresult', title: '处理结果', align:'center', valign:'middle',sortable: false},
//                    {field: 'disposemethod', title: '处理方法', align:'center', valign:'middle',sortable: false},
//                    {field: 'disposeremark', title: '处理备注', align:'center', valign:'middle',sortable: false},
                    ...
                    {field: 'rl_atpid', title: '操作',align:'center', sortable: false,
                        formatter: function (value, row, index) {
                            ...
                            a += '<a  class="btn btn-warning btn-xs" onclick="orders('+ inp +')">接单</a>&nbsp;';
                            a += '<a  class="btn btn-danger btn-xs" onclick="changerooter('+ inp +')">修改维修人</a>&nbsp;';
                            return a;
                        }
                    },
                ]
            ...
            onDblClickRow: function (row) {
                window.parent.ATP_BOX_OPEN('/index.php/Admin/RgRepairDelay/info?id=' + row['rl_atpid'], function() {
...
//                console.log(name+order);
    ...
    function detail(a){
        window.parent.ATP_BOX_OPEN('/index.php/Admin/RgRepairDelay/info?id=' + a, function () {
        });
    }

    function orders(a){
         window.parent.ATP_BOX_OPEN('/index.php/Admin/RgRepairDelay/orders?id=' + a,orderssubmitpassdata);
    }

    function changerooter(a){
        // alert(a);
         window.parent.ATP_BOX_OPEN('/index.php/Admin/RgRepairDelay/repairpeople?id='+a, repairpeoplesubmitdata);
    }

    function orderssubmitpassdata() {
        if (window.parent.ATP_BOX_VALIDATE()) {
            window.parent.ATP_BOX_CLOSE();
            var rl_atpid = $("#rl_atpid", parent.document).val();
            var rl_user_plandate = $("#rl_user_plandate", parent.document).val();
            $.ajax({
                url: '/index.php/Admin/RgRepairDelay/orderssubmitpassdata',
                type: 'POST',
                data: {"rl_atpid": rl_atpid, "rl_user_plandate": rl_user_plandate},
                async: false,
    ...
    function repairpeoplesubmitdata()
    {
            rl_atpid = $("#rl_atpid",parent.document).val();
            rl_empid = $("#rl_empid",parent.document).val();
            $.ajax({
            url: '/index.php/Admin/RgRepairDelay/repairpeoplesubmitdata',
            type: 'POST',
            data:  {"rl_atpid":rl_atpid,"rl_empid":rl_empid},
            async:false,
            success: function(data){
                // alert(data);
                alert("修改成功");
