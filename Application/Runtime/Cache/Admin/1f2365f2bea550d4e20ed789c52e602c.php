...
<script src="/Public/vendor/ueditor/third-party/jquery-1.10.2.js"></script>
...
<script>
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Datacontrastrank/getData?ids=<?php echo ($ids); ?>&selectParam=<?php echo ($selectParam); ?>&start=<?php echo ($start); ?>&end=<?php echo ($end); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//                $("#sys_dlg").load('/index.php/Admin/Datacontrastrank/edit?id=' + row['emp_atpid'], function() {
...
//        var ids = $("#ids").val();
        window.location.href='/index.php/Admin/Datacontrastrank/index?timerange=' + timerange + '&start='+start + '&end='+end+'&selectParam='+param +'&ids='+idstrings;
    });
</script>
<script>
    function choice(){
...
<script>
    $(function () {
        $.ajax({
            type: "GET",
            url: "/index.php/Admin/Datacontrastrank/getData?ids=<?php echo ($ids); ?>&selectParam=<?php echo ($selectParam); ?>&start=<?php echo ($start); ?>&end=<?php echo ($end); ?>",
            dataType: "json",
