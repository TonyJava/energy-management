...
<script>
    ...
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/RgGeneral/getAlarmData?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=<?php echo ($_GET['regionlevel']); ?>&tabindex=0&rgn_atpid=<?php echo ($_GET['rgn_atpid']); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//                           return "";
            ...
            onDblClickRow: function (row) {
                //     $("#sys_dlg").load('/index.php/Admin/RgGeneral/edit?id=' + row['alm_atpid'], function() {
...
//                console.log(name+order);
    ...
    {
        $.post('/index.php/Admin/RgGeneral/isignore?alm_atpid='+ id ,function(data){
            if(1 == data){
                if (confirm('忽略报警?')){
                    window.parent.ATP_BOX_OPEN("/index.php/Admin/RgGeneral/reason?alm_atpid="+id,reasonCallback);
                }
            }else{
                alert('只有待确认的报警才能忽略');
            }
    ...
    function reasonCallback() {
        ...
        $.post('/index.php/Admin/RgGeneral/ignore',{'alm_atpid':alm_atpid,'alm_disposeresult':alm_disposeresult},function (data) {
            if(1 == data){
                alert('报警忽略成功');
            }else{
                alert('报警忽略失败');
    ...
    {
        if (confirm('确认报警?')){
            $.post('/index.php/Admin/RgGeneral/iscomfirm?alm_atpid='+ id ,function(data){
                if(1 == data){
                    $.post('/index.php/Admin/RgGeneral/comfirm?alm_atpid=' + id,function(data){
                        if(1==data){
                            alert('报警确认成功');
                        }else{
                            alert('报警确认失败');
