    ...
    <div class="ibox float-e-margins">
        <div class="ibox-content" style="padding: 10px 10px 10px 10px;margin-bottom: -15px;">
            <div class="row row-lg">
                <div class="col-sm-12">
                    <i class="fa fa-hand-o-right"></i>&nbsp;当前位置:【基础数据管理】/【参数配置】
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Parammap/getData',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            height: 600,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            uniqueId: "pm_atpid",                     //每一行的唯一标识，一般为主键列
...
//            cardView: true,                    //是否显示详细视图
            ...
            columns: [
                [
                    ...
                    {field: 'p_name', title: '统计参数', align:'center', valign:'middle',sortable: true,
                        formatter: function (value, row, index) {
                        console.log(row);
                            var inp = "'"+  row.p_atpid +"'";
                            var value = value;
                            if (null != value){
                                var a = '<a onclick="checkparam ('+ inp +')">'+row['et_name']+'--'+value+'--'+row['p_shortname']+'--'+row['p_unit']+'</a>&nbsp;<br>';
                            }
                            return a;
                        }
                    },
                    {field: 'dmp_name', title: '设备参数', align:'center', valign:'middle',sortable: true,
                        formatter: function (value, row, index) {
                            console.log(row);
                            var inp = "'"+  row.dmp_atpid +"'";
                            var value = value;
                            if (null != value){
                                var a = '<a onclick="checkdevicemodelparam ('+ inp +')">'+row['cpy_name']+'--'+row['dm_name']+'--'+value+'--'+row['dmp_shortname']+'--'+row['dmp_style']+'--'+row['dmp_unit']+'</a>&nbsp;<br>';
                            }
                            return a;
                        }
                    },

                    {field:'pm_atpid',title:'主键',visible:false},
                    {field:'pm_atpcreatedatetime',title:'创建时间',visible:false},
                    {field:'pm_atpcreateuser',title:'创建人',visible:false},
                    {field:'pm_atplastmodifydatetime',title:'最后修改时间',visible:false},
                    {field:'pm_atplastmodifyuser',title:'最后修改人',visible:false},
                    {field:'pm_atpstatus',title:'数据状态',visible:false},
                    {field:'pm_atpsort',title:'数据排序',visible:false},
                    {field:'pm_atpremark',title:'数据备注',visible:false},
                    {field: 'pm_atpid', title: '操作',align:'center', sortable: false,width:100,
                        formatter: function (value, row, index) {
                            var inp = "'"+  value +"'";
            ...
            onDblClickRow: function (row) {
                $("#sys_dlg").load('/index.php/Admin/Parammap/edit?id=' + row['pm_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_add').on('click',function(){
        $("#sys_dlg").load('/index.php/Admin/Parammap/add', function() {
    ...
    function submitdata()
    {
        if ($.html5Validate.isAllpass($("#sys_dlg_form"))) {
            ...
            $("#sys_dlg_form").submit(function (e) {
                ...
                $.ajax({
                    url: '/index.php/Admin/Parammap/submit',
                    type: 'POST',
                    data: formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/index.php/Admin/Parammap/edit?id=' + tablerow[0]['pm_atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    function submitphonedata()
    {
        $("#sys_dlg_form").submit(function(e)
        {
            ...
            $.ajax({
                url: '/index.php/Admin/Parammap/submitphone',
                type: 'POST',
                data:  formData,
                mimeType:"multipart/form-data",
                contentType: false,
    ...
    {
        $("#sys_dlg").load('/index.php/Admin/Parammap/edit?id=' + id, function() {
            $('#sys_dlg_submit').on('click',submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    }
    ...
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/index.php/Admin/Parammap/del', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
                    $('#atpbiztable').bootstrapTable('refresh')
                }
                else {
    ...
    function checkparam(id){
        $("#sys_dlg").load('/index.php/Admin/Parammap/checkparam?id=' + id, function() {
            $("#sys_dlg").modal({backdrop: false});
        });
    }
    function checkdevicemodelparam(id){
        $("#sys_dlg").load('/index.php/Admin/Parammap/checkdevicemodelparam?id=' + id, function() {
            $("#sys_dlg").modal({backdrop: false});
        });
    }
