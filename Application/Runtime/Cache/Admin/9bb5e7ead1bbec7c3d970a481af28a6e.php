    ...
    <div class="ibox float-e-margins">
        <div class="ibox-content" style="padding: 10px 10px 10px 10px;margin-bottom: -15px;">
            <div class="row row-lg">
                <div class="col-sm-12">
                    <!--<?php if($_GET['type']== 'sd'){ ?>-->
                    <!--<i class="fa fa-hand-o-right"></i>&nbsp;当前位置:【基础数据管理】/【配置水电单价】-->
                    <!--<?php }else{ ?>-->
                    <!--<i class="fa fa-hand-o-right"></i>&nbsp;当前位置:【基础数据管理】/【字典浏览】-->
                    <!--<?php } ?>-->
                    <i class="fa fa-hand-o-right"></i>&nbsp;当前位置:【基础数据管理】/【字典浏览】
                </div>
            </div>
...
<script>
    GLOBAL_SEARCHNAME = "配置项";
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Config/getData?type=<?php echo ($_GET['type']); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            height: 600,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            uniqueId: "cfg_atpid",                     //每一行的唯一标识，一般为主键列
...
//                     {checkbox: true},
                    ...
                    {field: 'cfg_category', title: '类别',align:"center", sortable: true,width:200},
                    {field: 'cfg_key', title: '配置项',align:"center", sortable: true,width:200},
                    {field: 'cfg_value', title: '配置值', sortable: true,align:"center"},
//                    {field: 'cfg_atpid', title: '操作', sortable: false,align:"center",width:100,
//                        formatter: function (value, row, index) {
//                            var inp = "'"+  value +"'";
...
//                    },
                    ...
                    {field:'cfg_atpid',title:'主键',visible:false},
                    {field:'cfg_atpcreatedatetime',title:'创建时间',visible:false},
                    {field:'cfg_atpcreateuser',title:'创建人',visible:false},
                    {field:'cfg_atplastmodifydatetime',title:'最后修改时间',visible:false},
                    {field:'cfg_atplastmodifyuser',title:'最后修改人',visible:false},
                    {field:'cfg_atpstatus',title:'数据状态',visible:false},
                    {field:'cfg_atpsort',title:'数据排序',visible:false},
                    {field:'cfg_atpremark',title:'数据备注',visible:false},
                ]
            ],
            onDblClickRow: function (row) {
//                $("#sys_dlg").load('/index.php/Admin/Config/edit?id=' + row['cfg_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_add').on('click',function(){
        $("#sys_dlg").load('/index.php/Admin/Config/add', function() {
    ...
    function submitdata()
    {
        if ($.html5Validate.isAllpass($("#sys_dlg_form"))) {
            $('#sys_dlg').modal('hide');
            if ($("input[name='cfg_key']") .val() == "") {
                $("#c_key_p").css("display","block");
                return false;
            }
            $("#sys_dlg_form").submit(function (e) {
                ...
                $.ajax({
                    url: '/index.php/Admin/Config/submit',
                    type: 'POST',
                    data: formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/index.php/Admin/Config/edit?id=' + tablerow[0]['cfg_atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    {
        $("#sys_dlg").load('/index.php/Admin/Config/edit?id=' + id, function() {
            $('#sys_dlg_submit').on('click',submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    }
    ...
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/index.php/Admin/Config/del', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
                    $('#atpbiztable').bootstrapTable('refresh')
                }
                else {
