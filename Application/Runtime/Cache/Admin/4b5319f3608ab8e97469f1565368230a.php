    ...
    <div class="ibox float-e-margins">
        <div class="ibox-content" style="padding: 10px 10px 10px 10px;margin-bottom: -15px;">
            <div class="row row-lg">
                <div class="col-sm-12">
                    <i class="fa fa-hand-o-right"></i>&nbsp;当前位置:【<?php echo ($page); ?>】/【<?php echo ($page); ?>】/【房间管理】
...
<script>
    ...
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Usersideroom/getData',         //请求后台的URL（*）
...
//                $("#sys_dlg").load('/index.php/Admin/Usersideroom/edit?id=' + row['usr_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_add').on('click',function(){
        ...
        $("#sys_dlg").load('/index.php/Admin/Usersideroom/add?id='+ id +'&bs=<?php echo ($bs); ?>', function() {
    ...
    function submitdata()
    {
        $("#sys_dlg_form").submit(function(e)
        {
            ...
            $.ajax({
                url: '/index.php/Admin/Usersideroom/submit?bs=<?php echo ($bs); ?>',
                type: 'POST',
                data:  formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/index.php/Admin/Usersideroom/edit?id=' + tablerow[0]['usr_atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/index.php/Admin/Usersideroom/del', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
    ...
    $('#sys_del').on('click',function() {
        ...
        if (tablerow.length == 0) {
            ...
            if (confirm('确认删除' + tablerow.length + '条数据?')) {
                ...
                $.post('/index.php/Admin/Usersideroom/del?bs=<?php echo ($bs); ?>', {ids: ids.join(',')}, function (rep, status) {
                    if ('' == rep) {
                        location.reload();
//                        $('#atpbiztable').bootstrapTable('refresh')
                    }
