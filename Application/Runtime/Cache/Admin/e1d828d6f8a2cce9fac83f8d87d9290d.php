...
<script>
    ...
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Devicemodel/getData',         //请求后台的URL（*）
...
//            cardView: true,                    //是否显示详细视图
            ...
            onDblClickRow: function (row) {
                $("#sys_dlg").load('/index.php/Admin/Devicemodel/edit?id=' + row['dm_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_add').on('click',function(){
        $("#sys_dlg").load('/index.php/Admin/Devicemodel/add', function() {
    ...
    function submitdata()
    {
        if ($.html5Validate.isAllpass($("#sys_dlg_form"))) {
            ...
            $("#sys_dlg_form").submit(function (e) {
                ...
                $.ajax({
                    url: '/index.php/Admin/Devicemodel/submit',
                    type: 'POST',
                    data: formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/index.php/Admin/Devicemodel/edit?id=' + tablerow[0]['dm_atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    {
        $("#sys_dlg").load('/index.php/Admin/Devicemodel/edit?id=' + id, function() {
            $('#sys_dlg_submit').on('click',submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    }
    ...
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/index.php/Admin/Devicemodel/del', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
                    $('#atpbiztable').bootstrapTable('refresh')
                }
                else {
    ...
    function check(id){
        $("#sys_dlg").load('/index.php/Admin/Devicemodel/check?id=' + id, function() {
            $("#sys_dlg").modal({backdrop: false});
        });
    }

    function checkenergytype(id){
        $("#sys_dlg").load('/index.php/Admin/Devicemodel/checkenergytype?id=' + id, function() {
            $("#sys_dlg").modal({backdrop: false});
        });
    }
