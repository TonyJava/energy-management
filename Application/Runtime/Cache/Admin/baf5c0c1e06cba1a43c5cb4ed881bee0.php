            ...
            <div class="ibox-content">
                ...
                <div class="row row-lg">
                    <div class="col-sm-12">
                        ...
                        <div id="highchart" style="border:1px solid #ccc;margin-top: 20px" style="display: none"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Datacontrastreport/getData?ids=<?php echo ($ids); ?>&selectParam=<?php echo ($selectParam); ?>&start=<?php echo ($start); ?>&end=<?php echo ($end); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//                    {checkbox: true},
                ...
                <?php if(is_array($columns)): foreach($columns as $k=>$vo): ?>{field: '<?php echo ($vo); ?>', title: '<?php echo ($vo); ?>',align:'center', valign:'middle', sortable: false},<?php endforeach; endif; ?>
                ]
            ],
            onDblClickRow: function (row) {
//                $("#sys_dlg").load('/index.php/Admin/Datacontrastreport/edit?id=' + row['emp_atpid'], function() {
...
//        var ids = $("#ids").val();
        window.location.href = '/index.php/Admin/Datacontrastreport/index?timerange=' + timerange + '&start=' + start + '&end=' + end + '&selectParam=' + param + '&ids=' + idstrings;
    });
</script>
<script>
    function choice(){
...
$(function () {
    $.ajax({
        type: "GET",
        url: "/index.php/Admin/Datacontrastreport/getData?ids=<?php echo ($ids); ?>&selectParam=<?php echo ($selectParam); ?>&start=<?php echo ($start); ?>&end=<?php echo ($end); ?>",
        dataType: "json",
