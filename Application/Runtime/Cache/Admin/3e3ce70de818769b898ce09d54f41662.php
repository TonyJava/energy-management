    ...
    <div class="ibox float-e-margins">
        <div class="ibox-content" style="padding: 10px 10px 10px 10px;margin-bottom: -15px;">
            <div class="row row-lg">
                <div class="col-sm-12">
                    <i class="fa fa-hand-o-right"></i>&nbsp;当前位置:【<?php echo ($page); ?>】/【<?php echo ($page); ?>】/【表管理】
            ...
            <div class="row row-lg">
                <div class="col-sm-12">
                    <div id="atpbiztoolbar">
                     <button class="btn btn-info " type="button" onclick="sdf('<?php echo ($id); ?>')" ><i class="fa fa-pencil"></i>&nbsp;添加</button>
                     <!--  
                        ...
                        <input type="hidden" id="usersideid" name="" value="<?php echo ($id); ?>">
                    </div>
...
<script>
    ...
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Usersidewatch/getData',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            cardView: true,                    //是否显示详细视图
            ...
            columns: [
                [
                    ...
                    {field: 'rgn_name', title: '所有表', align:'center', valign:'middle',sortable: true},
...
//                $("#sys_dlg").load('/index.php/Admin/Usersidewatch/edit?id=' + row['usr_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_search').on('click',function() {
        ...
        $("#sys_dlg").load('/index.php/Admin/Usersidewatch/add?id=' , function() {
    ...
    function sdf(id){
        $("#sys_dlg").load('/index.php/Admin/Usersidewatch/add?id='+id +'&bs=<?php echo ($bs); ?>', function() {
           $("button[name ='bc']").attr("data-dismiss","");
            $('#sys_dlg_submit').on('click', submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    ...
    function submitdata()
    {
        $("#sys_dlg_form").submit(function(e)
        {
            ...
            $.ajax({
                url: '/index.php/Admin/Usersidewatch/submit',
                type: 'POST',
                data:  formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/index.php/Admin/Usersidewatch/edit?id=' + tablerow[0]['usr_atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    {
        window.location.href = '/index.php/Admin/Room?id=' + id  ;       
    }
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/index.php/Admin/Usersidewatch/del?bs=<?php echo ($bs); ?>', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
                    $('#atpbiztable').bootstrapTable('refresh')
                }
                else {
    ...
    $('#sys_del').on('click',function() {
        ...
        if (tablerow.length == 0) {
            ...
            if (confirm('确认删除' + tablerow.length + '条数据?')) {
                ...
                $.post('/index.php/Admin/Usersidewatch/del', {ids: ids.join(',')}, function (rep, status) {
                    if ('' == rep) {
                        $('#atpbiztable').bootstrapTable('refresh')
                    }
                    else {
    ...
    function backView()
    {
        window.location.href = '/index.php/Admin/Userside?bs=<?php echo ($bs); ?>';
    }
