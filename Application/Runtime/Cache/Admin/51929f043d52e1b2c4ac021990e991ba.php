    ...
    <div class="ibox float-e-margins">
        ...
        <div class="main-right">
            <div class="ibox-content">
                <div class="row row-lg">
                    <div class="col-sm-12">
                        <?php if($_GET['regiontype']== 'rg'): ?><iframe id="contentframe" src="/index.php/Admin/RgGeneral/indexregionroot?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=regionroot&tabindex=0&rgn_atpid=<?php echo ($rgn_atpid); ?>"  frameborder="no" width="100%" scrolling="yes" height="1400px" ></iframe>
                        <?php else: ?>
                            <iframe id="contentframe" src="/index.php/Admin/RgGeneral/indexregion?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=region&tabindex=0&rgn_atpid=<?php echo ($rgn_atpid); ?>"  frameborder="no" width="100%" scrolling="yes" height="1400px" ></iframe><?php endif; ?>
                    </div>
                </div>
            </div>
        </div>
...
<SCRIPT type="text/javascript">
    ...
    function onClick(e,treeId, treeNode) {
        ...
        if ('园区根' == treeNode.type) {
            $('#contentframe').attr('src',"/index.php/Admin/RgGeneral/indexregionroot?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=regionroot&tabindex=0&rgn_atpid=" + RGN_ATPID);
        }
        if ('园区' == treeNode.type) {
            $('#contentframe').attr('src',"/index.php/Admin/RgGeneral/indexregion?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=region&tabindex=0&rgn_atpid=" + RGN_ATPID);
        }
        if ('设备点' == treeNode.type) {
            $('#contentframe').attr('src',"/index.php/Admin/RgGeneral/indexdevicepoint?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=devicepoint&tabindex=0&rgn_atpid=" + RGN_ATPID);
        }
    }
...
<script>
   ...
   ATP_REGIONJUMbj = function (rgn_atpid) {
       ...
       $('#contentframe').attr('src', "/index.php/Admin/RgAlarmConfirm?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=" + regionlevel + "&tabindex=2&rgn_atpid=" + RGN_ATPID + "&rgn_atpidcurrent=" + rgn_atpid);
    ...
    ATP_REGIONJUMZH = function (devicepoint_rgn_atpid) {
        ...
        if(null!=devicepoint_rgn_atpid) {
            ...
            $('#contentframe').attr('src', "/index.php/Admin/RgGeneral/indexdevicepoint?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=" + tregionlevel + "&tabindex=0&rgn_atpid=" + devicepoint_rgn_atpid);
            return;
        }
        if ('园区根' == node.type) {
            $('#contentframe').attr('src',"/index.php/Admin/RgGeneral/indexregionroot?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=regionroot&tabindex=0&rgn_atpid=" + RGN_ATPID);
...
//    联动左树右页
   ATP_REGIONJUMP = function (region_id) {
       ...
       $('#contentframe').attr('src',"/index.php/Admin/RgGeneral/indexregion?regiontype=<?php echo ($_GET['regiontype']); ?>&snname=<?php echo ($_GET['snname']); ?>&regionlevel=region&tabindex=0&rgn_atpid=" + region_id);
   };
