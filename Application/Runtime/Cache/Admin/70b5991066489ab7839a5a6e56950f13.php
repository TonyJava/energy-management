            ...
            <div class="content-tabs">
                ...
                <nav class="page-tabs J_menuTabs">
                    <div class="page-tabs-content">
                        ...
                        <a target="_self" href="/index.php/Admin/Userside/Usersidepm?rgn_atpid=<?php echo ($rgn_atpid); ?>&us_atpid=<?php echo ($us_atpid); ?>"  class="onetab selected">报表及排名</a>
                ...
                <div class="col-sm-12">
                    <div id="atpbiztoolbar">
                        <form onkeydown="if(event.keyCode==13){return false;}"  id="sys_dlg_search_form" role="form" class="form-horizontal">
                            <input type="hidden" id="start" class="form-control" value="<?php echo ($start); ?>"  style="width: 120px; display:inline-block;" >
                            <input type="hidden" id="end" class="form-control"  value="<?php echo ($end); ?>" style="width: 120px; display:inline-block;">
                            ...
                            <button class="btn btn-warning" type="button" id="sys_index_search" style="margin-top: 5px;"><i class="fa fa-search"></i>&nbsp;查询</button>
							 <button class="btn btn-warning" type="button" id="sys_index_exp" style="margin-top: 5px;"><i class="fa fa-search"></i>&nbsp;导出</button>
                            <br>
                        </form>
                    </div>
...
<script>
    ...
    jumptab(currenttab);
    $(function () {
        $(".chosen-select").chosen({disable_search_threshold: 10, search_contains: false, width:'150px'});
    });
</script>
<script>
    ...
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Userside/getDatabaobiao?rgn_atpid=<?php echo ($rgn_atpid); ?>&start=<?php echo ($start); ?>&end=<?php echo ($end); ?>&us_atpid=<?php echo ($us_atpid); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//                    {field: 'rgn_name', title: '位置点', align:'center', valign:'middle',sortable: true,width: 140},
                    {field: 'ysl', title: '用水量', align:'center', valign:'middle',sortable: true,width: 140},
                    {field: 'dgl', title: '电量值',align:'center', valign:'middle', sortable: true,width: 140},
                    {field: 'yll', title: '用冷量',align:'center', valign:'middle', sortable: true,width: 140},
                    {field: 'ynl', title: '用暖量',align:'center', valign:'middle', sortable: true,width: 140},
...
//                $("#sys_dlg").load('/index.php/Admin/Userside/edit?id=' + row['us_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#btn_configyear').on('click',function() {
        window.parent.ATP_BOX_OPEN("/index.php/Admin/Userside/year",selectYearCallback);
    });
    function selectYearCallback() {
        if(window.parent.ATP_BOX_VALIDATE()){
            ...
            window.location.href='/index.php/Admin/Userside/index?start='+start + '&end='+end+'&rgn_atpid='+ rgn_atpid+'&us_atpid='+us_atpid;
        }
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    $('#btn_configyue').on('click',function() {
        window.parent.ATP_BOX_OPEN("/index.php/Admin/Userside/month",selectMonthCallback);
    });
    ...
    $('#btn_configday').on('click',function() {
        window.parent.ATP_BOX_OPEN("/index.php/Admin/Userside/day",selectDayCallback);
    });
    ...
    $('#sys_index_search').on('click', function () {
        ...
        var rgn_atpid = $('#rgn_atpid').val();
        var param = $('#param').val();
        window.location.href='/index.php/Admin/Userside/Usersidepm?start='+start + '&end='+end+'&rgn_atpid=<?php echo ($rgn_atpid); ?>'+'&us_atpid=<?php echo ($us_atpid); ?>';
    });
	//sys_index_exp
	 $('#sys_index_exp').on('click', function () {
        var start = $('#start').val();
        var end = $('#end').val();
        var rgn_atpid = $('#rgn_atpid').val();
        var param = $('#param').val();
        var windowurl='/index.php/Admin/Userside/exp?start='+start + '&end='+end+'&rgn_atpid=<?php echo ($rgn_atpid); ?>'+'&us_atpid=<?php echo ($us_atpid); ?>';
...
<script>
    $(function () {
        $.ajax({
            type: "GET",
            url: "/index.php/Admin/Userside/getDatabaobiao?rgn_atpid=<?php echo ($rgn_atpid); ?>&start=<?php echo ($start); ?>&end=<?php echo ($end); ?>&us_atpid=<?php echo ($us_atpid); ?>",
            dataType: "json",
