...
<script>
    ...
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Userside/getData?bs=<?php echo ($_GET['bs']); ?>',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            cardView: true,                    //是否显示详细视图
            ...
            onDblClickRow: function (row) {
                $("#sys_dlg").load('/index.php/Admin/Userside/edit?bs=<?php echo ($bs); ?>&id=' + row['us_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_add').on('click',function(){
        $("#sys_dlg").load('/index.php/Admin/Userside/add?bs=<?php echo ($bs); ?>', function() {
    ...
    function submitdata()
    {
        if ($.html5Validate.isAllpass($("#sys_dlg_form"))) {
            ...
            $("#sys_dlg_form").submit(function (e) {
                ...
                $.ajax({
                    url: '/index.php/Admin/Userside/submit',
                    type: 'POST',
                    data: formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/index.php/Admin/Userside/edit?id=' + tablerow[0]['us_atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    $('#sys_del').on('click',function() {
        ...
        if (tablerow.length == 0) {
            ...
            if (confirm('确认删除' + tablerow.length + '条数据?')) {
                ...
                // alert(ids);
                $.post('/index.php/Admin/Userside/del', {ids: ids.join(',')}, function (rep, status) {
                    if ('' == rep) {
                        $('#atpbiztable').bootstrapTable('refresh')
                    }
                    else {
    ...
    function ownRoom(id)
    {
        window.location.href = '/index.php/Admin/Usersideroom?id=' + id + '&bs=<?php echo ($bs); ?>';
    }
    function ownDevive(id)
    {
        window.location.href = '/index.php/Admin/Usersidewatch?id=' + id + '&bs=<?php echo ($bs); ?>';
    }
    function ownAll(id){
        window.location.href = '/index.php/Admin/Userside/Usersideinform?id=' + id + '&bs=<?php echo ($bs); ?>';
    }
