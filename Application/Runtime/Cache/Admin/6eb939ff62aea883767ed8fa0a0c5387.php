    ...
    <div class="ibox float-e-margins">
        <div class="ibox-content" style="padding: 10px 10px 10px 10px;margin-bottom: -15px;">
            <div class="row row-lg">
                <div class="col-sm-12">
                    <i class="fa fa-hand-o-right"></i>&nbsp;当前位置:【设备管理】/【厂家管理】
                </div>
            </div>
        </div>
    </div>
...
<script>
    GLOBAL_SEARCHNAME = "厂家名称";
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/index.php/Admin/Company/getData',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            height: 600,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            uniqueId: "cpy_atpid",                     //每一行的唯一标识，一般为主键列
...
//            cardView: true,                    //是否显示详细视图
            ...
            columns: [
                [
                    ...
                    {field: 'cpy_linkman', title: '联系人',align:'center', valign:'middle', sortable: true},
                    {field: 'cpy_linkphone', title: '联系方式',align:'center', valign:'middle', sortable: true},
                    {field: 'cpy_code', title: '公司邮编',align:'center', valign:'middle', sortable: true},
                    {field: 'cpy_email', title: '邮箱',align:'center', valign:'middle', sortable: true},
                    {field: 'cpy_officecode', title: '办公邮编',align:'center', valign:'middle', sortable: true},
                    {field: 'cpy_location', title: '企业地址',align:'center', valign:'middle', sortable: true},
                    {field: 'cpy_city', title: '所在城市',align:'center', valign:'middle', sortable: true},

                    {field:'cpy_atpid',title:'主键',visible:false},
                    {field:'cpy_atpcreatedatetime',title:'创建时间',visible:false},
                    {field:'cpy_atpcreateuser',title:'创建人',visible:false},
                    {field:'cpy_atplastmodifydatetime',title:'最后修改时间',visible:false},
                    {field:'cpy_atplastmodifyuser',title:'最后修改人',visible:false},
                    {field:'cpy_atpstatus',title:'数据状态',visible:false},
                    {field:'cpy_atpsort',title:'数据排序',visible:false},
                    {field:'cpy_atpremark',title:'数据备注',visible:false},
                    {field: 'cpy_atpid', title: '操作',align:'center',sortable: false,width:100,
                        formatter: function (value, row, index) {
                            var inp = "'"+  value +"'";
            ...
            onDblClickRow: function (row) {
                $("#sys_dlg").load('/index.php/Admin/Company/edit?id=' + row['cpy_atpid'], function() {
...
//                console.log(name+order);
    ...
    $('#sys_add').on('click',function(){
        $("#sys_dlg").load('/index.php/Admin/Company/add', function() {
    ...
    function submitdata()
    {
        if ($.html5Validate.isAllpass($("#sys_dlg_form"))) {
            ...
            $("#sys_dlg_form").submit(function (e) {
                ...
                $.ajax({
                    url: '/index.php/Admin/Company/submit',
                    type: 'POST',
                    data: formData,
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/index.php/Admin/Company/edit?id=' + tablerow[0]['cpy_atpid'], function() {
                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    {
        $("#sys_dlg").load('/index.php/Admin/Company/edit?id=' + id, function() {
            $('#sys_dlg_submit').on('click',submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    }
    ...
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/index.php/Admin/Company/del', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
                    $('#atpbiztable').bootstrapTable('refresh')
                }
                else {
    ...
    $('#sys_out').on('click',function(){
        window.open("/index.php/Admin/Company/expout");
    });
